...
const SnakeAnimation: React.FC = () => {
  const [segments, setSegments] = useState<Segment[]>([{ x: 0, y: 0 }]);
  const [direction, setDirection] = useState<'UP' | 'DOWN' | 'LEFT' | 'RIGHT' | 'PAUSE'>('RIGHT')
  const [isPaused, setIsPaused] = useState(false)

  const moveSnake = useCallback(() => {
    setSegments((prevSegments) => {
      const newSegments = prevSegments.map((segment, index) => {
        if (index === 0) {
          switch (direction) {
            case 'UP':
  ...
  useEffect(() => {
    const animationInterval = setInterval(moveSnake, 300);
